...
5. 成绩统计：个人成绩与管理端统计数据。
6. 监考回放：考试中自动采集切屏、摄像头断开、长时间不操作、网络恢复等异常事件，按时间轴回放；教师可复核事件（确认违规/排除误判）并调整违规扣分。
7. 异常申诉：学生可针对单个异常事件提交说明与截图；教师复核改判后，异常标记、生效分数与成绩统计自动联动更新。

## 角色权限
| 角色 | 可访问模块 |
|---|---|
| Student | 在线考试、我的成绩、监考回放（本人）、申诉中心（本人） |
| Teacher | 在线考试、我的成绩、题库管理、试卷管理、监考管理、申诉复核 |
| Admin | 全部功能（含数据统计） |
...
## 数据库说明
当前初始化后包含 12 张核心表（含用户、题目、试卷、考试记录、答案记录、监考异常事件、申诉等）。

已有数据卷升级时，`db-init` 会执行幂等迁移（自动创建 `proctoring_events`、`appeals` 表并为 `exam_records` 补充 `original_score`、`penalty_score` 字段），无需手工干预。
